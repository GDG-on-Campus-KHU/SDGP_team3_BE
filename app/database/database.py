...
async def execute_query(query: str, values: tuple = None) -> str:
    """
...
async def fetch_one(query: str, values: tuple = None) -> Dict[str, Any]:
    """
    단일 레코드 조회
    """
    pool = await get_pool()
...
async def fetch_all(query: str, values: tuple = None) -> List[Dict[str, Any]]:
    """
    여러 레코드 조회
    """
    pool = await get_pool()

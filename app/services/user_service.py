...
from app.models.user_model import User, UserCreate, UserUpdate, UserLogin
...
class UserService:
    ...
    async def create_user(user_data: UserCreate) -> Optional[User]:
        ...
        # 사용자 이름 중복 확인
        existing_username = await UserRepository.get_user_by_username(user_data.username)
    ...
    async def update_user(user_id: int, user_data: UserUpdate) -> Optional[User]:
        ...
        update_data = UserUpdate(**{k: v for k, v in user_data.model_dump().items() if v is not None})
